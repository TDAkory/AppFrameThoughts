...
- [One page note for system architecture designer](#one-page-note-for-system-architecture-designer)
  ...
  - [案例-软件架构](#案例-软件架构)
    ...
    - [例题一](#例题一-2)
    - [例子二](#例子二)
  - [案例-软件架构-数据相关](#案例-软件架构-数据相关)
    - [高频考点](#高频考点)
      - [**数据流图（DFD）**](#数据流图dfd)
      - [数据字典（DD）](#数据字典dd)
      - [二、E-R图（实体-联系图）](#二e-r图实体-联系图)
      - [UML 四种关系（表格）](#uml-四种关系表格)
        - [1. 图的分类](#1-图的分类)
        - [2. 重点图说明](#2-重点图说明)
    - [软件项目进度安排的两种图形方法](#软件项目进度安排的两种图形方法)
    - [例题一](#例题一-3)
      - [数据流图](#数据流图)
    - [例题二](#例题二-2)
  - [案例-数据库系统设计](#案例-数据库系统设计)
    - [高频知识](#高频知识)
      - [ORM技术](#orm技术)
      - [数据库类型比较](#数据库类型比较)
      - [缓存技术](#缓存技术)
      - [分布式锁](#分布式锁)
      - [数据库不规范化的四大问题](#数据库不规范化的四大问题)
      - [反规范化技术](#反规范化技术)
    - [例题一](#例题一-4)
    - [例题二](#例题二-3)
...
- **用例图**：描述用例、参与者及关系，用于系统语境和需求建模。用例关系有包含（<<include>>）、扩展（<<extend>>）、泛化。例如学生登记系统中，入学登记包含研讨会登记，学生登记扩展安全检查，登记学生与登记家庭成员存在泛化关系。
...
| 关键概念 | - 优点：直观展示任务时间线和并行性<br>- 缺点：无法清晰反映任务依赖关系，难以确定项目关键路径 | - 关键路径：从开始到结束的最长路径，决定项目最短工期<br>- 松弛时间：最迟开始时间 - 最早开始时间，关键路径上活动的松弛时间为0 |
...
补充数据流图(1)~(6)的内容，并说明DFD分层细化的**数据平衡原则**。

#### 数据流图

```
           项目信息表        事故及影响因素参数表        项目指标数据表
              ▲                    ▲                        ▲
              │                    │                        │
项目管理员─→项目信息维护         (5)影响因素录入         (1)项目指标填报─→安全员
              |       ↖            |                        ▼
              |     (4)安全评价得分  |               (2)项目指标审核─→安全副经理
              ▼              ↖     ▼                        ▼
          关联事故录入        (6)项目指标预警分析 <---- (3)项目指标确认─→项目经理
              ▲                    ▼
          指标参数表           指标预警分析表
```
